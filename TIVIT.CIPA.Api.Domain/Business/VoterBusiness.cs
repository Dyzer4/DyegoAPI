...
namespace TIVIT.CIPA.Api.Domain.Business
{
    public class VoterBusiness : IVoterBusiness
    {
        ...
        public async Task<Response<IEnumerable<VoterListResponse>>> GetByElectionIdAsync(int electionId)
        {
            var response = new Response<IEnumerable<VoterListResponse>>();
            ...
            response.Data = voters.Select(x => new VoterListResponse()
            {
                Id = x.Id,
